...
impl FireworksProvider {
    pub fn new(api_key: String, max_concurrent: usize, default_model: Option<String>) -> Self {
        Self { api_key, client: Client::new(), semaphore: Arc::new(Semaphore::new(max_concurrent)), default_model: default_model.unwrap_or_else(|| "accounts/fireworks/models/llama-v3-70b-instruct".to_string()) }
    }
...
impl LlmProvider for FireworksProvider {
    ...
    async fn available_models(&self) -> Result<Vec<ModelInfo>> {
        Ok(vec![
            ModelInfo { id: "accounts/fireworks/models/llama-v3-70b-instruct".to_string(), name: "Llama 3 70B".to_string(), context_window: 8_000, input_cost_per_1m: 0.64, output_cost_per_1m: 0.64 },
            ModelInfo { id: "accounts/fireworks/models/mixtral-8x7b-instruct".to_string(), name: "Mixtral 8x7B".to_string(), context_window: 32_000, input_cost_per_1m: 0.36, output_cost_per_1m: 0.36 },
        ])
    }
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let response = self.client.post(format!("{}/chat/completions", FIREWORKS_API_BASE)).header("Authorization", format!("Bearer {}", self.api_key)).json(&body).send().await.map_err(|e| AppError::InternalError(format!("Fireworks API failed: {}", e)))?;

        if !response.status().is_success() { return Err(AppError::InternalError(format!("Fireworks error ({})", response.status()))); }
        ...
        Ok(ChatResponse { content, provider: "fireworks".to_string(), model, usage, cost_eur, finish_reason: FinishReason::Stop })
    }
    ...
    async fn embed(&self, _texts: Vec<String>) -> Result<Vec<Vec<f32>>> { Err(AppError::InternalError("Fireworks embeddings not supported".to_string())) }
}
