/// Together AI provider (OpenAI-compatible, open-source models)
...
impl TogetherProvider {
    pub fn new(api_key: String, max_concurrent: usize, default_model: Option<String>) -> Self {
        Self { api_key, client: Client::new(), semaphore: Arc::new(Semaphore::new(max_concurrent)), default_model: default_model.unwrap_or_else(|| "meta-llama/Llama-3-70b-chat-hf".to_string()) }
    }

    fn calculate_cost(_model: &str, prompt_tokens: u32, completion_tokens: u32) -> f64 {
        let input_cost = (prompt_tokens as f64 / 1_000_000.0) * 0.64; // ~$0.70/M
        let output_cost = (completion_tokens as f64 / 1_000_000.0) * 0.64;
...
impl LlmProvider for TogetherProvider {
    ...
    async fn available_models(&self) -> Result<Vec<ModelInfo>> {
        Ok(vec![
            ModelInfo { id: "meta-llama/Llama-3-70b-chat-hf".to_string(), name: "Llama 3 70B".to_string(), context_window: 8_000, input_cost_per_1m: 0.64, output_cost_per_1m: 0.64 },
            ModelInfo { id: "mistralai/Mixtral-8x7B-Instruct-v0.1".to_string(), name: "Mixtral 8x7B".to_string(), context_window: 32_000, input_cost_per_1m: 0.45, output_cost_per_1m: 0.45 },
        ])
    }
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let response = self.client.post(format!("{}/chat/completions", TOGETHER_API_BASE)).header("Authorization", format!("Bearer {}", self.api_key)).json(&body).send().await.map_err(|e| AppError::InternalError(format!("Together API failed: {}", e)))?;

        if !response.status().is_success() { return Err(AppError::InternalError(format!("Together error ({})", response.status()))); }
        ...
        Ok(ChatResponse { content, provider: "together".to_string(), model, usage, cost_eur, finish_reason: FinishReason::Stop })
    }
    ...
    async fn embed(&self, _texts: Vec<String>) -> Result<Vec<Vec<f32>>> { Err(AppError::InternalError("Together embeddings not supported".to_string())) }
}
